...
tab_resutls_content = html.Div([
    ...
    dbc.Tabs([
        dbc.Tab([html.Div(id='outputbox', style={'marginTop': '20px'}),
                 dcc.RadioItems(id='graph-item',options={ 'height':'rate-height', 'dpdx':'rate-dp/dx'}, value='height', inline=True, 
                                style={'marginTop': '20px','marginRight': '20px'}, labelStyle={'marginRight': '20px', 'marginLeft': '20px'},
...
def generate_excel(n_clicks, curvename, curves):
    ...
    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
        df_curve = pd.DataFrame({
            'Slurry Rate (gal/min)': curve['q'],
            'Height (inches)': curve['h'],
            'Pressure Gradient (psi/ft)': curve['dpdx']
        })
        ...
        df_params = pd.DataFrame({
            'Parameter': ['Openhole (in)', 'Screen OD (in)', 'Screen ID (in)', 'Screen Centralizer (in)', 
                          ...
                          'Fluid density (ppg)', 'Fluid Viscosity (cP)', 'Gravel size (um)', 
            ...
            'Value': [curve['oh'], curve['screen_od'], curve['screen_id'], curve['screen_cd'], 
                      ...
                      curve['rho_f'], curve['mu_f'], curve['d_p'], 
                      curve['sg'], curve['ppa'], curve['qmin'], curve['hmax'], curve['model']]
        })
...
def update_outputbox(curves, graphitem):
    fig = go.Figure()
    ...
    fig.update_layout(title='Slurry Rate vs Height',
                      xaxis_title='Slurry Rate (gal/min)',
                      yaxis_title='Bed Height (inches)',
                      xaxis=dict(range=[0, 1.2 * max([c['qmax'] for c in curves])]),
                      width=800, height=600)
    if graphitem == 'dpdx':
        fig.update_layout(yaxis_title='Pressure Gradient (psi/ft)')
